...
let daysRepresented = (trips) => {

    let daysRepresented = 0;
    let lastTripBeginning = 0;
    let lastTripEnd = 0;
    let businessTrips = trips.sort((a, b)=> a[0] - b[0]);

    businessTrips.forEach((trip, i)=> {
        let tripLength = 0;
        if (trip[0] >= lastTripBeginning && trip[1] <= lastTripEnd){
            tripLength = 0;
        }else if (trip[0] <= lastTripEnd && trip[1] > lastTripEnd){
            tripLength = trip[1] - lastTripEnd;
            lastTripEnd = trip[1];
            lastTripBeginning = trip[0];
        }else {
            tripLength = trip[1] - trip[0] + 1;
            lastTripEnd = trip[1];
            lastTripBeginning = trip[0];
        }
        daysRepresented = daysRepresented + tripLength;
    })
